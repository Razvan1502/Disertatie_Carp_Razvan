...
public class TaxiDataStreamJob {
    public static void main(String[] args) throws Exception {
        // args[0] = mode (default 3), args[1] = slackSeconds override (optional)
        // Mode 3 default slack: 1500s (P95 trip duration). Examples: 500, 900, 1500, 2400, 3600
        int mode = (args.length > 0) ? Integer.parseInt(args[0]) : 3;
        long defaultSlack = (mode == 3) ? 1500 : 36;
        long slackSeconds = (args.length > 1) ? Long.parseLong(args[1]) : defaultSlack;
        String sinkName = (args.length > 1)
                ? String.format("taxi_watermark_m%d_s%d", mode, slackSeconds)
                : "taxi_watermark_m" + mode;

        final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(1);

        String uniqueGroupId = "taxi-watermark-group-" + System.currentTimeMillis();
        ...
        System.out.printf("🚀 Taxi Fixed Watermark | Mode %d | Slack: %ds | Sink: %s%n",
                mode, slackSeconds, sinkName);

        env.execute("Taxi - Fixed Watermark Mode " + mode);
    }
}

...
public class TaxiInOrderJob {
    public static void main(String[] args) throws Exception {
        final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(1);

        String uniqueGroupId = "taxi-inorder-group-" + System.currentTimeMillis();
        KafkaSource<SmartHomeEvent> source = KafkaSource.<SmartHomeEvent>builder()
                .setBootstrapServers("localhost:9092")
                .setTopics("iot-sensor-data-inorder")
        ...
        WatermarkStrategy<SmartHomeEvent> watermarkStrategy = WatermarkStrategy
                .<SmartHomeEvent>forMonotonousTimestamps()
        ...
        results.addSink(new TaxiInfluxDBSink("taxi_inorder"));
        System.out.println("🚀 Taxi In-Order Job pornit! (pickup-sorted Mode 4, slack 1s — referință in-order)");
        env.execute("Taxi - In-Order Baseline");
    }
}
